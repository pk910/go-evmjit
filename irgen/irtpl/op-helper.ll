...
{{- if .StackCheck }}
%l{{ .Id }}_stack_in_check = icmp ult i64 %l{{ .Id }}_stack_in_val, {{ .Count }}
...
{{- define "stack-check" }}
{{ if .Verbose }}; stack overflow check ({{ .Count }} words){{- end }}
%l{{ .Id }}_stack_out_val = load i64, i64* %stack_position_ptr, align 8
%l{{ .Id }}_stack_out_check = icmp ugt i64 %l{{ .Id }}_stack_out_val, {{ sub .MaxStack .Count }}
br i1 %l{{ .Id }}_stack_out_check, label %l{{ .Id }}_stack_out_err1, label %l{{ .Id }}_stack_out_ok
l{{ .Id }}_stack_out_err1:
  store i64 {{ .Pc }}, i64* %pc_ptr
